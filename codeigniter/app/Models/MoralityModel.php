...
class MoralityModel extends Model
{
    public function getMorality()
    {
        ...
        $query = $db->query('SELECT xh, xm, sfzh, dyjf, dyxf, zzsxsz, jthd, yjxxx, xystwh, gyld, rcxw, ssbx, jlf, kf, bz FROM morality');
    ...
    {
        $db = \Config\Database::connect();
        $query = $db->query("UPDATE morality SET zzsxsz='".$data['zzsxsz']."', jthd='".$data['jthd']."', yjxxx='".$data['yjxxx']."', xystwh='".$data['xystwh']."', gyld='".$data['gyld']."', rcxw='".$data['rcxw']."', ssbx='".$data['ssbx']."', jlf='".$data['jlf']."', kf='".$data['kf']."' WHERE sfzh='".$data['sfzh']."';");
